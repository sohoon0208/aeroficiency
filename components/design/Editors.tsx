  ...
  return (
    <div className="editor-stack airfoil-editor">
      <section className="control-group">
        ...
        <p className="airfoil-catalog-note"><strong>Geometry catalogue only.</strong> The same normalized contour drives the loft, wing box, 2D section view, zero-lift angle, and pitching moment. Selecting a supercritical profile does not add compressibility or wave-drag physics; the analytic polar remains an attached-flow estimate unless you import matching tables.</p>
...
export function CaseEditor({ flightCase, constraints, editable, onUpdate }: { flightCase: FlightCase; constraints: DesignConstraints; editable: boolean; onUpdate: (patch: Partial<AngleSweepPatch>) => DomainResult<unknown> }) {
  ...
  return (
    <div className="editor-stack">
      ...
      <div className="locked-case"><span aria-hidden="true">◇</span><p>Target lift and atmospheric conditions stay shared and fixed. The AoA exploration range is shared and editable so every design is compared over identical angles.</p></div>
    </div>
  );
}
