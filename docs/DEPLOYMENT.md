...
Aeroficiency builds to a Cloudflare Worker through the checked-in Vinext/Vite configuration. It has no application database, storage bucket, login, backend API, third-party runtime call, or secret. The normal UI remains usable when WebMCP is unavailable.
...
## Managed private preview

The project also supports a private ChatGPT Sites deployment through `.openai/hosting.json`. That path is useful for owner-only verification before public release. Public access still requires a separate explicit approval and the same live-origin checks.
...
Never copy files from the parent challenge-planning directory into this repository. Private travel, eligibility, credentials, correspondence, and attachment records are deliberately outside the Git root.
