...
- [ ] Final scope matches `README.md` and no cut feature is implied.
- [ ] The UI, snapshots, validation record, eval fixture, and demo all identify solver `aerociency-0.2.0`.
- [ ] Fresh reset produces the exact deterministic baseline and no fabricated result.
- [ ] Manual baseline → candidate → edit → analyze → compare workflow passes.
- [ ] Exactly eight Site Tools register once and remain absent when WebMCP is unavailable.
- [ ] Human and agent mutations use the same commands and actor labels are truthful.
- [ ] Baseline protection, idempotency, revision conflict, stale result, abort, failure, and non-convergence paths pass.
- [ ] Solver snapshots contain only finite JSON values or documented nulls.
- [ ] Every visible metric, plot, constraint, warning, and activity value traces to committed state.
- [ ] Geometry, Aero loads, and Structure modes are readable and accurate.
- [ ] 1440×900, tablet, 390×844, keyboard, focus, reduced motion, and 200% zoom checks pass.
- [ ] `npm ci` succeeds from a clean local copy.
- [ ] `npm run lint` passes.
- [ ] `npm run typecheck` passes.
- [ ] `npm run test:run` passes.
- [ ] `npm run licenses:check` passes.
- [ ] `npm run build` passes without unexplained warnings.
- [ ] `npm audit --audit-level=high` passes or every exception has a written reachability decision.
- [ ] No secret, token, credential, local absolute path, or private attachment is tracked.
- [ ] Root MIT license, notices, security policy, provenance, assumptions, validation, tool docs, evals, submission copy, and demo script are present.
- [ ] Original favicon and social preview are used; third-party rights are documented.
- [ ] Local Git repository is rooted at `aerociency/`, with normal dated history.
...
- [ ] Feasible candidate numbers match the frozen solver within documented tolerance.
