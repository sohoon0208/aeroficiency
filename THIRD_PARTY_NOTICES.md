...
- Ten airfoil coordinate geometries are bundled from the University of Illinois Urbana-Champaign Applied Aerodynamics Group's [UIUC Airfoil Data Site](https://m-selig.ae.illinois.edu/ads/coord_database.html): Clark Y; S1210, S1223, and S2048; SG6041, SG6042, and SG6043; and NASA SC(2)-0410, SC(2)-0412, and SC(2)-0612. Each runtime definition retains its source filename. The SC(2) coordinates originate from NASA Technical Paper 2969; alternate points are retained to stay within the application's bounded coordinate limit. These numerical geometries are redistributed with source attribution and are not relicensed by Aeroficiency's MIT software license.
- NACA four-digit coordinates are computed from the documented public-domain equations rather than copied from a dataset.

Technical references inform the model and validation methodology; their text, figures, data, and code are not redistributed. See `docs/MODEL_ASSUMPTIONS.md` and `docs/VALIDATION.md` for citations.
